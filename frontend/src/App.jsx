...
const initialTasks = [
  { id: 1, title: 'Design onboarding flow', project: 'Website Redesign', status: 'in-progress', priority: 'High', due: 'Today', dueDate: '2026-09-23', assignee: 'KF', tone: 'indigo', comments: 4 },
  ...
  { id: 5, title: 'Review authentication API', project: 'Mobile App', status: 'in-progress', priority: 'High', due: 'Sep 25', dueDate: '2026-09-25', assignee: 'KF', tone: 'emerald', comments: 5 },
...
const people = [
  { initials: 'KF', name: 'Kaiqbal F.', color: 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/60 dark:text-indigo-200' },
...
function App() {
  ...
  const [workspaceLoading, setWorkspaceLoading] = useState(() => Boolean(localStorage.getItem('taskflow_token')))
  ...
  const [workspaceCodeCopied, setWorkspaceCodeCopied] = useState(false)
  ...
  useEffect(() => {
    if (session?.demo) {
      const timer = window.setTimeout(() => {
        const demoWorkspace = { id: 'demo-workspace', name: 'Northstar Product Team', owner_id: 'demo-kaiqbal', join_code: 'NSTAR123', members: demoMembers }
  ...
  const currentUserInitials = initialsFromName(session?.user?.name)
  const myTasks = useMemo(() => session?.demo ? tasks.filter((task) => task.assignee === currentUserInitials) : tasks.filter((task) => task.assigneeId != null && String(task.assigneeId) === String(currentUserId)), [currentUserId, currentUserInitials, session?.demo, tasks])
  ...
  async function inviteMember(email) {
    ...
    const updatedWorkspace = await api.addMember(session.token, apiContext.workspace.id, { email })
  ...
  async function copyWorkspaceCode() {
    const code = apiContext?.workspace?.join_code
    if (!code || !navigator.clipboard) return
    await navigator.clipboard.writeText(code)
    setWorkspaceCodeCopied(true)
    window.setTimeout(() => setWorkspaceCodeCopied(false), 1600)
  }

  async function createWorkspace(name) {
    if (!name.trim()) return
    if (!session?.token) {
      const created = { id: `demo-workspace-${Date.now()}`, name, owner_id: session.user?.id, join_code: `DEMO${String(Date.now()).slice(-4)}`, projects_count: 0, members: apiContext?.workspace?.members ?? [] }
          ...
          <main className="mx-auto max-w-[1600px] p-5 sm:p-8">
            {activeNav === 'Overview' ? <>
              ...
              <section className="mt-8 grid gap-6 xl:grid-cols-[minmax(0,1fr)_310px]">
                <div className="min-w-0"><div className="mb-4 flex flex-col justify-between gap-3 sm:flex-row sm:items-center"><div><h3 className="text-base font-bold">Task overview</h3><p className="mt-1 text-xs text-slate-400">Drag and drop tasks to update their status.</p></div><div className="flex items-center gap-2"><div className="relative"><span className="pointer-events-none absolute left-3 top-1/2 z-10 h-2 w-2 -translate-y-1/2 rounded-full bg-indigo-500" /><select aria-label="Filter tasks by project" value={selectedProject} onChange={(event) => setSelectedProject(event.target.value)} className="w-full appearance-none rounded-lg border border-slate-200 bg-white py-2 pl-8 pr-8 text-xs font-medium text-slate-600 outline-none transition focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 dark:focus:ring-indigo-950">{projectOptions.map((project) => <option key={project.name} value={project.name}>{project.name}</option>)}</select><ChevronDown size={14} className="pointer-events-none absolute right-2.5 top-1/2 -translate-y-1/2 text-slate-400" /></div><button className="rounded-lg border border-slate-200 bg-white p-2 text-slate-400 dark:border-slate-700 dark:bg-slate-900"><MoreHorizontal size={16} /></button></div></div>
